...
# Precision is chosen against the size budget AND against what the numbers mean: 5 significant
# figures is already far beyond what sampling noise at these shot counts supports, and
# decoder throughput on one machine is a 3-figure quantity at best.
PRECISION = {"decoder_seconds_per_shot": 3}
...
def normalise(row: dict) -> dict:
    ...
    for field in ROW_FIELDS:
        value = row.get(field)
        out[field] = sig(value, PRECISION.get(field, 5)) if isinstance(value, float) else value
    return out
...
def main() -> int:
    ...
    manifest = {
        ...
        "seed_rule": (
            "seed = master_seed + index. index is the 0-based position in the frozen task "
            "enumeration, which is generated by nesting the axes below in this exact order: "
            "threshold_grid = for code in codes: for distance in distances: "
            "for noise_model in noise_models: for p in p_grids[noise_model]; then "
            "decoder_comparison = for distance in distances: for p in p_grids['circuit_level'] "
            "with code='repetition', noise_model='circuit_level'. Each row carries its own seed, "
            "so the enumeration is not restated here."
        ),
        "task_count": len(plan),
